...
public class LoginWindow extends JFrame implements LoginViewContract {
    ...
    public void navigateToDashboard(User user) {
        ...
        domain.repository.StudentRepository studentRepository = new data.repository.MockStudentRepository();
        domain.usecase.StudentUseCase studentUseCase = new domain.usecase.StudentUseCase(studentRepository);
        HandlerDashboardWindow dashboardPresenter = new HandlerDashboardWindow(studentUseCase);

        // JOptionPane.showMessageDialog(null, "Navegar al Dashboard!");
        DashboardWindow dashboard = new DashboardWindow(dashboardPresenter, user);
